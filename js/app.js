document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (scheduleView) {
    ...
    // Afficher uniquement deux semaines : la semaine en cours + la semaine suivante.
    // La fenêtre se décale automatiquement chaque lundi.
    const viewStart = new Date(today);
    const day = viewStart.getDay();
    const daysSinceMonday = day === 0 ? 6 : day - 1;
    viewStart.setDate(viewStart.getDate() - daysSinceMonday);
  ...
  quiz.addEventListener('submit', function(e){
    ...
    box.innerHTML=`<strong>Note : ${score}/20</strong><span>${percent}% — ${message}</span>`;
